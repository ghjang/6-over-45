...
*   **의미**: "과거의 모든 기록"입니다.
*   **업데이트**: `latest_lottery_result.json`의 내용을 읽어서 이 파일의 끝에 추가하고, 통계를 갱신합니다.
...
*   **업데이트**: 모든 통계가 갱신된 후, 알고리즘을 통해 **완전히 새롭게 생성**됩니다.
...
3.  **`update_guesses.py` 실행**:
    ...
    *   갱신된 `frequency.json` 통계를 바탕으로, **1211회차를 위한 새로운 예측 번호**를 생성하여 `next_guess.json`을 덮어씁니다.
